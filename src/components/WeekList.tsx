...
function EventBadge({ evt }: { evt: NonNullable<Week['weekEvent']> }) {
  const colors = {
    ...
    camp:        'bg-slate-100 border-slate-300 text-slate-700',
  }
  return (
...
function VolBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex items-center gap-1.5">
      <div className="w-10 h-2 rounded-sm overflow-hidden" style={{ background: 'var(--bc-border)' }}>
  ...
  return (
    <div className="space-y-5">
      {months.map(({ month, weeks: mWeeks }) => (
        <div key={month}>
          {/* Month header — red left border accent */}
          <div className="flex items-center gap-2 mb-2 px-1">
            <div style={{ width: 3, height: '1rem', background: 'var(--bc-red)', borderRadius: 2, flexShrink: 0 }} />
            <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--bc-muted)' }}>
              {month}
            </h3>
          </div>

          <div className="rounded-xl overflow-hidden border bg-white" style={{ borderColor: 'var(--bc-border)' }}>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs font-semibold border-b" style={{ background: 'var(--bc-charcoal)', borderColor: 'var(--bc-steel)', color: '#888' }}>
                  ...
                  <th className="px-4 py-2.5 text-center">Int</th>
                  {planMode === 'detailed' && <>
                    <th className="px-4 py-2.5 text-center">Rnd</th>
                    <th className="px-4 py-2.5 text-center">Tech</th>
                    <th className="px-4 py-2.5 text-center">S&C</th>
                    <th className="px-4 py-2.5 text-center">Cardio</th>
                  </>}
                  {athlete?.showWeightCycles && <th className="px-4 py-2.5 text-center">Weight</th>}
                  {athlete?.showCardioCycles && <th className="px-4 py-2.5 text-center">Cardio</th>}
                  <th className="px-4 py-2.5 text-left">Events</th>
                  <th className="px-4 py-2.5 text-left">Focus / Notes</th>
              ...
              <tbody>
                {mWeeks.map((w, i) => (
                  <tr key={w.weekNumber}
                    className={`border-b last:border-0 cursor-pointer transition-colors hover:bg-red-50/40 ${
                    ...
                    <td className="px-4 py-2.5 text-xs text-gray-500 whitespace-nowrap">
                      ...
                      {w.location && w.location !== 'home' && (
                        <span className="ml-1 inline-flex items-center">
                          <MapPin size={10} className={w.location === 'camp' ? 'text-slate-400' : 'text-orange-400'} />
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2.5">
                      <VolBar value={w.intensity} color="var(--bc-red)" />
                    </td>
                    {planMode === 'detailed' && <>
                    ...
                    <td className="px-2 py-2.5">
                      <Edit2 size={13} className="text-gray-300 hover:text-red-500 transition-colors" />
                    </td>
                  </tr>
                ))}
              </tbody>
